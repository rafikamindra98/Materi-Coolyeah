...
train_exog = train[['DayOfWeek', 'Month']]
test_exog = test[['DayOfWeek', 'Month']]
...
train[['Pengangkutan']] = scaler.fit_transform(train[['Pengangkutan']])
test[['Pengangkutan']] = scaler.transform(test[['Pengangkutan']])

# Normalization using MinMaxScaler
from sklearn.preprocessing import MinMaxScaler
scaler = MinMaxScaler()
train_scaled = scaler.fit_transform(train[['Pengangkutan']])
test_scaled = scaler.transform(test[['Pengangkutan']])

# Pisahkan kembali data setelah normalisasi
train['Pengangkutan'] = train_scaled[:, 0]
test['Pengangkutan'] = test_scaled[:, 0]
...
for param in itertools.product(p, d, q):
    try:
        model = ARIMA(train['Pengangkutan'], order=param)
        model_fit = model.fit()
...
# Train ARIMA model with best parameters (no exogenous)
model = ARIMA(train['Pengangkutan'], order=best_cfg)
model_fit = model.fit()
