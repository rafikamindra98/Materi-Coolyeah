...
# Include the 'Holiday' column as part of the features for training and testing
train_features = train[['Transportation', 'Holiday']]
test_features = test[['Transportation', 'Holiday']]

# Scale the features
train_scaled = scaler.fit_transform(train_features)
test_scaled = scaler.transform(test_features)

# Adjust the create_dataset function to include a time step for LSTM compatibility
def create_dataset(dataset, time_step=1):
    X, y = [], []
    for i in range(len(dataset) - time_step):
        X.append(dataset[i:(i + time_step), 1:])  # Use all columns except the first ('Pengangkutan') as features
        y.append(dataset[i + time_step, 0])  # The first column ('Pengangkutan') is the target
    return np.array(X), np.array(y)

# Update the dataset preparation with a time step
time_step = 1  # Adjust this value as needed
...
# Ensure the input data is reshaped correctly for LSTM
