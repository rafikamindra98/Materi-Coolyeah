...
# Buat kolom 'Libur' jika belum ada
if 'Libur' not in data.columns:
    data['Libur'] = data.index.weekday
    data['Libur'] = data['Libur'].apply(lambda x: 1 if x >= 5 else 0)

# Tambahkan fitur baru: is_holiday (libur nasional)
indonesia_holidays = holidays.Indonesia(years=[2023, 2024, 2025])  # Sesuaikan tahun data
...
X_train = train[['Libur', 'Is_Holiday', 'Month']]  # Exogenous multiple

y_test = test['Pengangkutan']
X_test = test[['Libur', 'Is_Holiday', 'Month']]

# 5. Auto-ARIMA dengan seasonality
print("Mencari parameter SARIMAX terbaik dengan fitur tambahan...")
...
print('\nEvaluasi Model SARIMAX dengan fitur tambahan:')
print(f'MAE  : {mae:.2f}')
...
plt.title('Forecasting Pengangkutan Penumpang SARIMAX (dengan Is_Holiday & Month)')
plt.xlabel('Waktu')
