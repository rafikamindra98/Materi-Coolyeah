...
# Prepare data for Vanilla Transformer (vTRF)
...
# Vanilla Transformer Model (vTRF)

def transformer_encoder(inputs, head_size, num_heads, ff_dim, dropout=0.1):
    # Multi-head Attention layer
    attention = keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=head_size)(inputs, inputs)
    attention = keras.layers.Dropout(dropout)(attention)
    ...
    return keras.layers.LayerNormalization(epsilon=1e-6)(attention + ff)

# Build Transformer model
def build_transformer_model(input_shape, head_size=64, num_heads=4, ff_dim=128, num_blocks=2, dropout=0.1):
...
# Reshape predictions to 2D by removing the time step dimension
train_predict = train_predict.reshape(-1, 1)  # Shape: (samples, 1)
test_predict = test_predict.reshape(-1, 1)    # Shape: (samples, 1)
...
plt.title('Transformer Model Predictions vs Actual Data')
plt.xlabel('Date')
