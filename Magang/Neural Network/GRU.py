...
# Build GRU model (as an equivalent to MGU)
model = keras.Sequential([
    keras.layers.Input(shape=(1, train_scaled.shape[1])),  # Shape: (timesteps, features)
    keras.layers.GRU(100, return_sequences=True),  # Replaced LSTM with GRU
    keras.layers.Dropout(0.2),
    keras.layers.GRU(50),  # Replaced LSTM with GRU
    keras.layers.Dropout(0.2),
...
plt.title('GRU Model Predictions vs Actual Data')
plt.xlabel('Date')
