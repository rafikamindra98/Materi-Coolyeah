...
# Reshape input to be [samples, timesteps, features] for transformer
X_train = X_train.reshape((X_train.shape[0], 1, X_train.shape[1]))  # Shape: (samples, timesteps, features)
X_test = X_test.reshape((X_test.shape[0], 1, X_test.shape[1]))      # Shape: (samples, timesteps, features)

# PatchTST Model
class PatchTST(keras.Model):
    def __init__(self, embed_dim=32, num_heads=2, ff_dim=64, num_layers=2, dropout=0.1):
        ...
        # Define layers
        self.embedding = keras.layers.Dense(embed_dim)  # Project input features to embedding dimension
        self.attention_layers = [keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=embed_dim) for _ in range(num_layers)]
        self.attention_dropouts = [keras.layers.Dropout(dropout) for _ in range(num_layers)]
        self.attention_norms = [keras.layers.LayerNormalization(epsilon=1e-6) for _ in range(num_layers)]
        
        # Two-layer feed-forward network
        self.ffn_layers = [[
            keras.layers.Dense(ff_dim, activation="relu"),
            keras.layers.Dense(embed_dim)  # Project back to embedding dimension
        ] for _ in range(num_layers)]
        self.ffn_dropouts = [keras.layers.Dropout(dropout) for _ in range(num_layers)]
        self.ffn_norms = [keras.layers.LayerNormalization(epsilon=1e-6) for _ in range(num_layers)]
    ...
    def call(self, inputs):
        ...
        x = self.embedding(x)  # Shape: (batch_size, timesteps, embed_dim)

        # Apply multi-head attention layers
        for i in range(self.num_layers):
            # Self-attention
            attention_output = self.attention_layers[i](x, x)
            attention_output = self.attention_dropouts[i](attention_output)
            x = x + attention_output
            x = self.attention_norms[i](x)
            
            # Feed-forward network
            ffn_output = x
            for layer in self.ffn_layers[i]:
                ffn_output = layer(ffn_output)
            ffn_output = self.ffn_dropouts[i](ffn_output)
            x = x + ffn_output
...
# Build model
model = PatchTST(embed_dim=32, num_heads=2, ff_dim=64, num_layers=2, dropout=0.1)
...
plt.title('PatchTST Model Predictions vs Actual Data')
plt.xlabel('Date')
